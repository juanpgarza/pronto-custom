...
class ProcurementGroup(models.Model):
    ...
    def run_smart_scheduler(self, picking_id = 0):
        ...
        dias_registrar_actividad = int(self.env['ir.config_parameter'].sudo().search([('key','=','pronto_stock_scheduler.dias_registrar_actividad')]).value)
        dias_reservar = int(self.env['ir.config_parameter'].sudo().search([('key','=','pronto_stock_scheduler.dias_reservar')]).value)
        ...
        activity_type_id = self.env.ref('pronto_stock_scheduler.contactar_cliente_reserva_stock')
        ...
        if picking_id == 0:
            ...
            activity_type_id = self.env.ref('pronto_stock_scheduler.reconfirmar_retiro')
            for picking in picking_ids:
                activity = picking._schedule_activity(activity_type_id)
